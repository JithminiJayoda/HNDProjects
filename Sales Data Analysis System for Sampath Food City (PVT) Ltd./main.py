...
def monthly_sales_analysis(branch_id):
    ...
    plt.hist(branch_sales, bins=10, edgecolor='black', color='hotpink')
...
def all_branches_monthly_sales_analysis():
    ...
    for sale in sales_data:
        try:
          branch_id = sale[0]
          # Ensure branch_id is a string before accessing the dictionary
          if isinstance(branch_id, str):
            monthly_sales[branch_id] += int(sale[2])
          else:
            print(f"Warning: Skipped sale with invalid Branch ID: {branch_id}")
        except KeyError:
          print(f"Error: Branch ID '{branch_id}' not found in branches data.")

    ####### not sure about this but it works xD ########
    branch_ids, sales_amounts = zip(*monthly_sales.items())

    plt.figure(figsize=(10, 6))
    plt.bar(branch_ids, sales_amounts, color='red', edgecolor='black')
